...
function SavedInboxDetails({ inbox, on_update, space_name }: IDetails) {
    ...
    async function handle_submit(event: FormEvent<HTMLFormElement>): Promise<void> {
        ...
        try {
            set_loading(true);
            const response = await fetch(`https://kliboardapi.railway.internal:5000/api/inbox/${space_name}`, {
                method: "PATCH",
    ...
    async function fetch_data(name: string) {
        try {
            ...
            const response = await fetch(`https://kliboardapi.railway.internal:5000/api/inbox/${name}`, { method: "GET" });
            const json = await response.json();
